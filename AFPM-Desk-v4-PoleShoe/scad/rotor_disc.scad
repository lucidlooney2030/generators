...
// Front face: magnets. Under magnets: pole-shoe pockets. Back: steel back-iron recess.

module rotor_disc(side="A") {
  // Floor between pole-shoe pocket bottom and steel recess top
  // magnet from front, then pole shoe deeper, steel from back
  shoe_z = rotor_thickness - magnet_pocket_depth - pole_shoe_pocket_depth;
  ...
  difference() {
    ...
    // pole-shoe pockets under each magnet (concentric, slightly smaller)
    for (i = [0:magnet_count-1]) {
      angle = i * 360 / magnet_count;
      rotate([0,0,angle])
        translate([magnet_pcd/2, 0, shoe_z])
          cylinder(h=pole_shoe_pocket_depth+0.1,
                   d=pole_shoe_d + pole_shoe_pocket_xy);
    }
    ...
    for (i = [0:magnet_count-1]) {
      ...
      rotate([0,0,angle])
        translate([magnet_pcd/2, 0, steel_recess_depth + 0.4]) {
          if (is_N) {
            cube([7, 1.4, 0.8], center=true);
          } else {
            cube([7, 1.4, 0.8], center=true);
            cube([1.4, 7, 0.8], center=true);
          }
        }
    }
